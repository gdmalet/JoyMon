...
DWORD g_ctltimerstart, g_ctltimertest;
char g_MsgText[512];
...
// 				PostMessage(hDlg,WM_CLOSE,0,0L); 
        ...
        case WM_TIMER:
			...
			if ( wParam == 69 ) {
				...
				DWORD ctltimernow = GetTickCount();	// verify the high performance timer

				if ( QueryPerformanceCounter( &timernow ) ) {
					float elapsed = (float)(timernow.QuadPart - g_timertest.QuadPart) / g_timerfreq.QuadPart;
					float ctlelapsed = (ctltimernow - g_ctltimertest) / 1000.0;
					_snprintf(g_MsgText, sizeof g_MsgText, "%6.3f second test timer apparently ran for %6.3f seconds.",
						ctlelapsed, elapsed);
					if ( fabs(ctlelapsed - elapsed) > 0.1 ) {
	...
	if ( PollJoystick( js ) == S_OK ) {
		...
		if ( state == Up &&	g_JoystickButton ) {
			...
			if ( !g_bWriting && timenow - lastclick >= 3 ) {
				...
				g_ctltimerstart = GetTickCount();	// a control, to verify `high performance' timers.
				g_timertest.QuadPart = 0;
...
bool WriteToFile( void )
{
	...
	if ( ! QueryPerformanceCounter( &timernow ) )
		return false;
	DWORD ctltimernow = GetTickCount();

	// The so-called `high performance' timer can do weird things, including jump, so be careful
	// it is meaningful. If not, use GetTickCount(), which may well be good enough anyway.
	float elapsed = (float)(timernow.QuadPart - g_timerstart.QuadPart) / g_timerfreq.QuadPart;
	float ctlelapsed = (ctltimernow - g_ctltimerstart) / 1000.0;

fprintf(fp, "# elapsed HPT %6.3f, LPT %6.3f (%6.3f)\n", elapsed, ctlelapsed, (float)elapsed-ctlelapsed);
	if (fabs(ctlelapsed - elapsed) > 0.1) {
		fprintf(fp, "# High performance timer is drifting; switching to low performance: elapsed HPT %6.3f, LPT %6.3f\n",
			elapsed, ctlelapsed);
		elapsed = ctlelapsed;
	}
